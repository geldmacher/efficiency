...
- When different fixes sharing one premise repeatedly leave the same symptom unresolved or fail the same acceptance criterion, recommend naming that premise before another correction. Choose the cheapest observation that distinguishes a false premise from an implementation defect. Repeated failure is a reason to investigate, not proof that the premise is false.
- Inspect callers, states, or roles only when the hypothesis depends on them. Apply findings within an existing debugging or fix assignment; report necessary scope expansion as a proposal. In an assessment-only task, recommend the next observation without performing diagnosis or repair.
