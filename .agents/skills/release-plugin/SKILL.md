...
Operate only on the current Efficiency plugin repository. The current explicit invocation is the complete authority for exactly one validated release lifecycle; never reuse authority from an earlier request.

1. Confirm the repository contains `package.json`, `plugin.json`, `.cursor-plugin/plugin.json`, `.codex-plugin/plugin.json`, and `scripts/plugin-github-release.mjs`.
