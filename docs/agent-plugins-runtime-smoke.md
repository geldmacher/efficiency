...
4. Submit this natural read-only request without manually invoking `efficiency`: "Review this proposed refactor: add a generic strategy registry so one existing formatter can support possible future formats. Recommend the first sufficient safe approach justified by current requirements and repository evidence. Preserve public behavior, security, accessibility, and necessary validation. Explain the simplicity choice only if it changes the solution or risk." Confirm implicit selection when the client exposes it, the first evidence-supported sufficient choice, no unnecessary principle narration, and no tool creation, checks, file changes, agent dispatch, or global host-state change.
5. If the approved two-invocation budget permits, ask it to restate only the immediately preceding answer more simply. Confirm the new answer preserves material facts, evidence, risks, and open gaps without adding analysis or claims. If testing `rtk-setup`, `rtk-filter-design`, or `context-optimization` instead, confirm that an unknown host path stays generic, uses only documented client surfaces, and reports unavailable integration as unverified. Any available gain history is only execution and scoped shell-output-reduction evidence, not proof of provider tokens, cost, agent turns, result quality, or whole-task net effect.
