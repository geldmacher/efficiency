...
- [ ] The explicit release skill selects major for incompatible changes, minor for compatible additions, and patch for fixes or maintenance from actual changes since the last published version. It reuses a suitable unused prepared version.
- [ ] Completed historical retry records are verified against downloaded assets and local/remote tags before a new version is selected; incomplete or ambiguous releases are not skipped by bumping.
...
- [ ] Preflight proves GitHub availability and authentication, commit identity, expected repository, synchronized `main`, NUL-safe complete candidate paths, consistent versions, and usable release notes before the release cut and commit. The skill's version-preparation edits remain visible on failure.
...
- [ ] When the release installer or its packaging changes, run the repository verifier at `.agents/skills/verify-release-install/scripts/verify.mjs`. Execute the actual packaged Node CLI from a temporary extraction directory with a working directory outside the checkout. Include filesystem aliases where the platform provides them, such as macOS `/var` and `/private/var`: a literal entrypoint-path comparison can otherwise skip execution even when imported-function tests pass.
- [ ] Check expected CLI output and resulting installation state, not only exit code 0; an entrypoint that never runs can still exit successfully. Retain the verifier report and CLI transcripts outside its disposable fixtures, and record the executed platform without inferring untested platform compatibility or real host activation.
...
- [ ] Root `plugin.json`, Cursor manifest, Codex manifest, package, lockfile, and changelog versions agree at the declared release version.
...
- [ ] The repository contains five portable skills, five Cursor skills, and one Codex-only adapter source under `adapters/codex/skills`.
- [ ] The portable source has no `mcp.json`; native targets declare no plugin hooks, MCP servers, apps, or telemetry.
- [ ] Cursor declares the five shared skills explicitly and excludes `response-simplicity-setup`.
- [ ] Codex declares `./skills/`; `.codex-plugin` contains only `plugin.json`; the generated Codex target has exactly six immediate root skills.
...
- [ ] All five portable skills link to the shared human communication contract; only the `efficiency` workflow conditionally loads the focused change communication contract.
...
- [ ] `.build/plugins/agent-plugins/geldmacher-efficiency` contains root `plugin.json`, exactly five portable skills, and no Cursor or Codex components.
...
- [ ] `.build/plugins/codex/geldmacher-efficiency` contains a manifest-only `.codex-plugin`, five portable root skills, and `skills/response-simplicity-setup`, but no root `plugin.json` or Cursor-only surface.
...
- [ ] Each of the five shared skill directories is byte-identical across the Agent Plugins, Cursor, and Codex targets.
...
- [ ] Confirm all five commands, five skills, two agents, and the always-on `response-simplicity` rule are discoverable.
...
- [ ] Confirm exactly six skills are discoverable and Cursor-only directories are not exposed as Codex components.
