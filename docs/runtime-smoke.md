...
| --- | --- | --- | --- |
| E1 | Implementiere eine JavaScript-Funktion, die einen Namen trimmt und bei leerem Ergebnis „Gast“ liefert. Gib nur den Code in der Antwort aus; ändere keine Dateien. | Implement a JavaScript function that trims a name and returns "Guest" if empty. Return the code in your answer; do not change files. | No `efficiency` selection. With the always-applied guidance active (the Cursor rule, or configured Codex guidance), a small implementation with no new abstraction or file changes. |
| D1 | Diese Funktion soll bei leerem Namen „Gast“ liefern, gibt aber `undefined` zurück: `function greet(name) { return name.trim() \|\| undefined; }`. Behebe den Fehler nur in der Antwort; ändere keine Dateien. | This function should return "Guest" for an empty name but returns `undefined`: `function greet(name) { return name.trim() \|\| undefined; }`. Fix it in your answer only; do not change files. | No `efficiency` selection. With the always-applied guidance active, its debugging pointer applies silently: name the observed symptom, give the smallest fix and the check that shows it; no unrelated refactor or efficiency narration. |
| V1 | Erweitere `formatDate(date)` um eine optionale Zeitzone und nenne am Ende, was du geprüft hast. Nur Code und Prüfliste in der Antwort; ändere keine Dateien. | Extend `formatDate(date)` with an optional time zone and list what you checked at the end. Code and check list in your answer only; do not change files. | No `efficiency` selection. With the always-applied guidance active, its verification pointer applies silently: the smallest set of facts covering the time-zone and default-path risks; no general risk inventory. |
