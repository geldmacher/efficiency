...
import { PLUGIN_NAME, EXPECTED_REPOSITORY, RELEASE_HOSTS, sha256, canonicalJson, assetName, receiptForProvenance, validateProvenance, exactKeys, shaField, contentSummary } from "../skills/install-new-release-from-repo/scripts/release-format.mjs";
import { buildPluginTargets } from "./build-plugin-targets.mjs";

const scriptPath = fileURLToPath(import.meta.url);
export { PLUGIN_NAME, EXPECTED_REPOSITORY, RELEASE_HOSTS, canonicalJson, receiptForProvenance };
...
function sourceState(root, runner, { requireClean = false, requireReleaseCut = false } = {}) {
  ...
  const manifests = {
    ...
    agent_plugins: readJson(join(root, "plugin.json"), "Agent Plugins manifest"),
  };
  ...
  if (repository !== EXPECTED_REPOSITORY) throw new Error(`repository must be ${EXPECTED_REPOSITORY}, received ${repository}`);
  for (const [target, manifest] of Object.entries(manifests)) {
  ...
  if (packageLock.version !== version || packageLock.packages?.[""]?.version !== version) {
    throw new Error(`package-lock.json version differs from ${version}`);
  }
  let notes;
...
export function inspectReleaseTarget(directory, host, version) {
  ...
  return contentSummary(entries.map((entry) => ({ ...entry, bytes: readFileSync(entry.path) })));
}

function createDeterministicArchive(source, archive, runner) {
  const temporary = mkdtempSync(join(tmpdir(), "efficiency-release-archive-"));
...
function atomicWriteText(path, source) {
  ...
  try {
    ...
    writeFileSync(staged, source, { mode });
...
function nulGitPaths(root, args, runner) {
  return runCheckedRaw(runner, "git", args, { cwd: root }, `git ${args.join(" ")}`)
    .split("\0")
    .filter((value) => value !== "");
}
...
function changedRepositoryPaths(root, runner) {
  const values = [
    nulGitPaths(root, ["diff", "--name-only", "-z"], runner),
    nulGitPaths(root, ["diff", "--cached", "--name-only", "-z"], runner),
    nulGitPaths(root, ["ls-files", "--others", "--exclude-standard", "-z"], runner),
  ];
...
export function prepareRelease({
  ...
  const buildTemporary = mkdtempSync(join(tmpdir(), "efficiency-release-build-"));
...
function verifyReleaseMetadata(view, directory, provenance) {
  ...
  if (view.name !== `Efficiency ${provenance.version}`) throw new Error("GitHub release title differs from the prepared release");
...
function verifyRemoteRelease(root, runner, directory, provenance, expectedHashes) {
  ...
  const downloadRoot = mkdtempSync(join(tmpdir(), "efficiency-release-readback-"));
  try {
...
export function publishRelease(receipt, {
  ...
  const publicationDirectory = mkdtempSync(join(tmpdir(), "efficiency-release-publish-"));
  try {
    ...
    const argumentsList = [
      ...
      "--repo", provenance.repository,
      "--verify-tag",
      "--title", `Efficiency ${provenance.version}`,
...
function localTagCommit(root, runner, tag) {
  ...
  const commit = optionalGit(root, ["show-ref", "--verify", "--hash", reference], runner);
  if (!commit) return null;
  const type = git(root, ["cat-file", "-t", reference], runner);
  if (type !== "commit") throw new Error(`local tag ${tag} must be lightweight, not annotated`);
...
function readReleaseState(releaseRoot, source) {
  ...
  exactKeys(state, [
    "base_commit", "commit_created", "kind", "phase", "release_commit", "repository", "schema", "tag", "tree_sha", "version",
  ], "release retry state");
  ...
  if (!["prepared", "committed"].includes(state.phase)) throw new Error("release retry state phase is invalid");
  if (typeof state.commit_created !== "boolean") throw new Error("release retry state commit_created must be boolean");
  shaField(state.base_commit, "release retry base commit", [40, 64]);
  if (state.phase === "prepared") {
    if (!state.commit_created || state.release_commit !== null) throw new Error("prepared release retry state must precede one release commit");
  } else {
...
function writeReleaseState(releaseRoot, source, baseCommit, {
  ...
  atomicWriteText(releaseStatePath(releaseRoot, source.version), canonicalJson(state));
  return state;
}

function assertExactRetryState(source, remoteMain, state, root, runner, releaseRoot) {
  ...
  if (state.tree_sha !== source.tree) throw new Error("release retry state does not bind the current Git tree");
  if (state.commit_created) {
    const parent = git(root, ["rev-parse", "HEAD^"], runner);
    const subject = git(root, ["show", "-s", "--format=%s", "HEAD"], runner);
    if (parent !== state.base_commit || subject !== `Release ${source.tag}`) {
      throw new Error("local release retry commit is not the exact harness-created release commit");
    }
    if (source.commit !== remoteMain && state.base_commit !== remoteMain) {
      throw new Error("release retry state does not bind origin/main");
    }
  } else if (state.base_commit !== source.commit) {
    throw new Error("no-commit release retry state does not bind the current main");
  }
  if (state.phase === "prepared") {
    state = writeReleaseState(releaseRoot, source, state.base_commit, { phase: "committed", commitCreated: true });
  } else if (state.release_commit !== source.commit) {
    throw new Error("release retry state does not bind the current release commit");
  }
  return state;
}
...
function preserveIndex(root, runner) {
  ...
  const temporary = mkdtempSync(join(tmpdir(), "efficiency-release-index-"));
...
export function completeRelease({
  ...
  let retryState = readReleaseState(releaseRoot, source);
  const retryingLocalCommit = source.commit !== baseline;
  if (retryingLocalCommit) {
    retryState = assertExactRetryState(source, baseline, retryState, resolvedRoot, runner, releaseRoot);
  } else if (retryState?.phase === "committed") {
    retryState = assertExactRetryState(source, baseline, retryState, resolvedRoot, runner, releaseRoot);
  }
  ...
  if (existingRelease && !existingRemoteTag) {
    throw new Error(`GitHub release ${source.tag} exists without its lightweight remote tag; mixed remote state will not be repaired`);
  }
  ...
  try {
    ...
    if (afterGatePaths.length > 0) {
      ...
      try {
        ...
        const staged = nulGitPaths(resolvedRoot, ["diff", "--cached", "--name-only", "-z"], runner).sort();
        const stagedTree = git(resolvedRoot, ["write-tree"], runner);
        ...
        if (stagedTree === headTree) {
          ...
          retryState = writeReleaseState(releaseRoot, source, baseCommit, {
            phase: "prepared",
            commitCreated: true,
            releaseCommit: null,
            treeSha: stagedTree,
        ...
        if (!indexRestored) index.restore();
        throw error;
      } finally {
        index.close();
      }
      source = sourceState(resolvedRoot, runner, { requireClean: true, requireReleaseCut: true });
      retryState = writeReleaseState(releaseRoot, source, baseCommit, { commitCreated });
    } else {
      source = sourceState(resolvedRoot, runner, { requireClean: true, requireReleaseCut: true });
      if (!retryState && !existingLocalTag && !existingRemoteTag && !existingRelease) {
        retryState = writeReleaseState(releaseRoot, source, baseCommit, { commitCreated: false });
      }
    }
  } catch (error) {
  ...
  const refsCurrent = remoteMain === source.commit && remoteTag === source.commit;
  const canPushFromBase = remoteMain === baseCommit
    && remoteTag === null
    && (remoteMain !== source.commit || retryState?.commit_created === false);
  if (!refsCurrent && !canPushFromBase) {
    throw new Error(`mixed remote state for ${source.tag}; origin/main=${remoteMain}, tag=${remoteTag ?? "missing"}; no remote state was changed`);
  }
  let pushedAtomically = false;
  if (canPushFromBase) {
  ...
  return {
    ...
    commit_sha: source.commit,
    commit_created: commitCreated,
    pushed_atomically: pushedAtomically,
