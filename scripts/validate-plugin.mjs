...
export function validatePlugin(root = defaultRoot) {
  ...
  if (!existsSync(manifestPath)) return [...failures, ".cursor-plugin/plugin.json is missing"];
  ...
  for (const field of ["rules", "hooks", "mcpServers"]) {
    if (!manifest[field]) continue;
    for (const value of componentPaths(manifest[field])) {
      if (typeof value === "string") validateDeclaredPath(rootPath, rootReal, value, `plugin.json ${field}`, failures);
    }
  }
  ...
  const commands = listFilesRecursive(join(rootPath, "commands"), (file) => [".md", ".txt"].includes(extname(file)))
  ...
  for (const [type, records] of [["commands", commands], ["agents", agents], ["skills", skills]]) {
    if (records.length === 0) failures.push(`${type}: no components found`);
  }
  ...
  for (const record of agents) {
    requireString(record.fields, "name", record.label, failures);
    requireString(record.fields, "description", record.label, failures);
    requireString(record.fields, "model", record.label, failures);
