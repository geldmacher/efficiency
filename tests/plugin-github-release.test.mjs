...
function repositoryFixture() {
  const fixtureRoot = mkdtempSync(join(tmpdir(), "efficiency-github-release-test-"));
  const repository = join(fixtureRoot, "repository");
  mkdirSync(repository);
  writeJson(join(repository, "package.json"), { name: "fixture", version, private: true });
  writeJson(join(repository, "package-lock.json"), { name: "fixture", version, lockfileVersion: 3, packages: { "": { name: "fixture", version } } });
  const common = { name: PLUGIN_NAME, version, repository: "https://github.com/geldmacher/efficiency" };
  writeJson(join(repository, ".cursor-plugin", "plugin.json"), common);
  writeJson(join(repository, ".codex-plugin", "plugin.json"), common);
  writeJson(join(repository, "plugin.json"), common);
  ...
  git(repository, "remote", "add", "origin", "git@github.com:geldmacher/efficiency.git");
  git(repository, "config", "user.name", "Efficiency Release Test");
  git(repository, "config", "user.email", "efficiency-release@invalid.local");
  git(repository, "add", ".");
  execFileSync("git", [
    "-c", "user.name=Efficiency Release Test",
    "-c", "user.email=efficiency-release@invalid.local",
...
function productionSnapshotFixture() {
  const fixtureRoot = mkdtempSync(join(tmpdir(), "efficiency-production-release-test-"));
  ...
  const changelog = createReleaseCut(readFileSync(changelogPath, "utf8"), packageJson.version);
...
function completeReleaseRunner(item, {
  ...
  const runner = (command, args, options = {}) => {
    ...
    if (command === "git" && args[0] === "commit") {
      if (commitFails) return { status: 1, stdout: "", stderr: "commit hook rejected release" };
      const result = defaultRunner(command, args, options);
      if (partialMainAfterCommit && result.status === 0) remoteMain = git(item.repository, "rev-parse", "HEAD");
      return result;
    }
    ...
    if (args[0] === "release" && args[1] === "create") {
      ...
      releaseView = {
        ...
        url: `https://github.com/geldmacher/efficiency/releases/tag/${args[2]}`,
      };
...
test("release Skill is explicit-only and stays in parity with the Cursor command and package scripts", () => {
  ...
  assert.match(command, /\.\.\/\.\.\/\.agents\/skills\/release-plugin\/SKILL\.md/);
  ...
  assert.match(skill, /npm run release:plugin/);
  ...
  const output = mkdtempSync(join(tmpdir(), "efficiency-release-surface-test-"));
  try {
    const built = buildPluginTargets(output);
    for (const target of [built["agent-plugins"].path, built.cursor.path, built.codex.path]) {
      assert.equal(existsSync(join(target, ".agents", "skills", "release-plugin")), false);
      assert.equal(existsSync(join(target, ".cursor", "commands", "release-plugin.md")), false);
      assert.equal(existsSync(join(target, "scripts", "plugin-github-release.mjs")), false);
    }
  } finally {
...
test("complete release preserves NUL-delimited candidate paths containing newlines", () => {
  const item = repositoryFixture();
  try {
    const unusual = "line\nbreak.txt";
    writeFileSync(join(item.repository, unusual), "included\n");
    const stub = completeReleaseRunner(item);
    const result = completeRelease({
      root: item.repository,
      releaseRoot: join(item.fixtureRoot, "releases"),
    ...
    const tracked = execFileSync("git", ["ls-files", "-z"], { cwd: item.repository }).toString("utf8").split("\0");
    assert.equal(result.status, "published");
    assert.ok(tracked.includes(unusual));
  } finally {
    rmSync(item.fixtureRoot, { recursive: true, force: true });
  }
});

test("a prepared retry state recovers the exact release commit after the commit boundary", () => {
  const item = repositoryFixture();
  const releaseRoot = join(item.fixtureRoot, "releases");
  try {
    writeFileSync(join(item.repository, "pending-state.txt"), "retry\n");
    const stub = completeReleaseRunner(item, { pushFails: true });
    const options = {
      root: item.repository,
      releaseRoot,
    ...
    assert.throws(() => completeRelease(options), /atomic main and release tag push failed/);
    const statePath = join(releaseRoot, `.release-v${version}.json`);
    const state = JSON.parse(readFileSync(statePath, "utf8"));
    state.phase = "prepared";
    state.release_commit = null;
    ...
    assert.equal(recovered.release_commit, result.commit_sha);
  } finally {
    rmSync(item.fixtureRoot, { recursive: true, force: true });
  }
});

test("complete release refuses mixed remote main and tag state without repair", () => {
  const item = repositoryFixture();
  try {
    writeFileSync(join(item.repository, "mixed-state.txt"), "candidate\n");
    const stub = completeReleaseRunner(item, { partialMainAfterCommit: true });
    assert.throws(() => completeRelease({
      root: item.repository,
      releaseRoot: join(item.fixtureRoot, "releases"),
      runner: stub.runner,
    ...
    assert.equal(stub.calls.some(([command, args]) => command === "git" && args[0] === "push"), false);
    assert.notEqual(spawnSync("git", ["show-ref", "--verify", `refs/tags/v${version}`], { cwd: item.repository }).status, 0);
  } finally {
    rmSync(item.fixtureRoot, { recursive: true, force: true });
  }
});

test("complete release rejects annotated local and remote tags before release mutation", () => {
  const local = repositoryFixture();
  try {
    git(local.repository, "tag", "-a", `v${version}`, "-m", "annotated");
    const stub = completeReleaseRunner(local);
    assert.throws(() => completeRelease({
      root: local.repository,
      releaseRoot: join(local.fixtureRoot, "releases"),
      runner: stub.runner,
      targetBuilder,
      releaseGate: () => ({ command: "npm run release-check", result: "passed" }),
    }), /must be lightweight, not annotated/);
  } finally {
    rmSync(local.fixtureRoot, { recursive: true, force: true });
  }

  const remote = repositoryFixture();
  try {
    const stub = completeReleaseRunner(remote);
    const commit = git(remote.repository, "rev-parse", "HEAD");
    const runner = (command, args, options = {}) => {
      if (command === "git" && args[0] === "ls-remote" && args.includes("--tags")) {
        return {
          status: 0,
          stdout: `${"a".repeat(40)}\trefs/tags/v${version}\n${commit}\trefs/tags/v${version}^{}\n`,
          stderr: "",
        };
      }
      return stub.runner(command, args, options);
    };
    assert.throws(() => completeRelease({
      root: remote.repository,
      releaseRoot: join(remote.fixtureRoot, "releases"),
      runner,
      targetBuilder,
      releaseGate: () => ({ command: "npm run release-check", result: "passed" }),
    }), /must be lightweight, not annotated/);
  } finally {
    rmSync(remote.fixtureRoot, { recursive: true, force: true });
  }
});

test("final staged tree equality with HEAD produces a no-op release commit decision", () => {
  const item = repositoryFixture();
  try {
    const path = join(item.repository, "normalizable.txt");
    writeFileSync(path, "original\n");
    git(item.repository, "add", "normalizable.txt");
    git(item.repository, "commit", "--quiet", "-m", "add normalizable fixture");
    git(item.repository, "update-ref", "refs/remotes/origin/main", "HEAD");
    writeFileSync(path, "candidate\n");
    const stub = completeReleaseRunner(item);
    const runner = (command, args, options = {}) => {
      if (command === "git" && args[0] === "add" && args[1] === "--all") writeFileSync(path, "original\n");
      return stub.runner(command, args, options);
    };
    const originalHead = git(item.repository, "rev-parse", "HEAD");
    const result = completeRelease({
...
test("prepare rejects dirty state, manifest version drift, and a failed release gate", () => {
  ...
  try {
    ...
    writeJson(join(item.repository, ".codex-plugin", "plugin.json"), {
      name: PLUGIN_NAME,
      version: "9.9.9",
...
test("independent preparations produce byte-identical closed Cursor and Codex release sets", () => {
  ...
  try {
    ...
    assert.deepEqual(readdirSync(first.directory).sort(), [
      "RELEASE_NOTES.md",
      "SHA256SUMS",
      `geldmacher-efficiency-codex-v${version}.zip`,
      `geldmacher-efficiency-cursor-v${version}.zip`,
      "provenance.json",
    ]);
...
test("two production-target preparations from one clean release-cut snapshot are byte-identical", () => {
  ...
  try {
    ...
    assert.ok(first.provenance.targets.cursor.file_count > 30);
    assert.ok(first.provenance.targets.codex.file_count > 25);
